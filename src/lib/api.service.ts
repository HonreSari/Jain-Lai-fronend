...
export const libraryService = {
  ...
  addToLibrary: async (seriesId: number): Promise<void> => {
    await api.post(`/library/${seriesId}`)
  },
  ...
  checkInLibrary: async (seriesId: number): Promise<CheckLibraryResponse> => {
    const response = await api.get<CheckLibraryResponse>(
      `/library/${seriesId}/check`
    )
    return response.data
  },
}
